...
class Actnorm(nn.Module):
    def __init__(self, num_chs):
        ...
        size = [1, num_chs, 1, 1]
        bias = torch.normal(mean=torch.zeros(*size), std=torch.ones(*size)*0.1)
        logs = torch.normal(mean=torch.zeros(*size), std=torch.ones(*size)*0.1)
        self.register_parameter('bias', nn.Parameter(torch.Tensor(bias), requires_grad=True))
        self.register_parameter('log_scale', nn.Parameter(torch.Tensor(logs), requires_grad=True))
