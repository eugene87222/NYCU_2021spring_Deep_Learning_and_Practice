...
def calc_z_shapes(in_sz, flow_depth, num_levels):
    z_shapes = []
    n_chs = in_sz[0]
    img_sz = in_sz[1]
...
def calc_loss(log_p, log_det, in_sz, n_bins):
    n_pixel = in_sz[0] * in_sz[1] * in_sz[2]
...
def evaluate(model, loader, eval_model, z_shapes, temp):
    model.eval()
    avg_acc = 0
    gen_images = None
    with torch.no_grad():
        for _, conds in enumerate(loader):
            ...
            for shape in z_shapes:
                z = torch.randn(conds.shape[0], *shape) * temp
                z_sample.append(z.to(device))
            conds = conds.to(device)
            fake_images = model.reverse(z_sample, conds)
            ...
            avg_acc += acc
    avg_acc /= len(loader)
    result = [avg_acc, gen_images.cpu().data]
    return result
...
def train(args, model, optimizer, log_dir, cpt_dir, result_dir):
    ...
    z_sample = []
    z_shapes = calc_z_shapes(args.in_sz, args.flow_depth, args.num_levels)
    for shape in z_shapes:
    ...
    for i in pbar_iter:
        ...
        if (i+1)%args.eval_interval == 0:
            ...
            with torch.no_grad():
                if args.dataset == 'CLEVR':
                    acc, gen_images = evaluate(model, test_loader, eval_model, z_shapes, args.temp)
                    logger.add_scalar('iter/acc', acc, i+1)
                    save_image(
                        gen_images.cpu().data,
...
if __name__ == '__main__':
    ...
    # model
    parser.add_argument('--in_sz', type=int, nargs='+', default=[3, 64, 64])
    ...
    task_name = 'CondGLOW-{}-cond_sz{}-cond_fc_fts{}-affine_conv_chs{}-flow_depth{}-num_levels{}-num_iters{}-lr{}-beta1{}-beta2{}-bs{}'.format(
        args.dataset, args.cond_sz, args.cond_fc_fts, args.affine_conv_chs,
        args.flow_depth, args.num_levels, args.num_iters, args.lr,
        args.beta1, args.beta1, args.bs)
